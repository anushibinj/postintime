...
- Each social account is **Manual** (toggle published locally) or **Webhook**. Webhook accounts POST multipart `title`, `caption`, and optional `media` to the configured URL (optional HTTP Basic). Text fields are sent as `text/plain` parts; the request is buffered with `Content-Length` so receivers like n8n reliably see all parts. A successful webhook marks that account published on the post; failures return `PUBLISH_FAILED` with the response body.
